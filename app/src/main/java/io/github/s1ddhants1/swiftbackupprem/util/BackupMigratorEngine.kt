...
object BackupMigratorEngine {
    ...
    fun migrate(config: MigrationConfig, context: Context? = null): MigrationResult {
        ...
        fun str(resId: Int, vararg args: Any): String =
            appContext?.getString(resId, *args) ?: when (resId) {
                R.string.step_migrating -> "Migrating"
                R.string.step_processing_apps -> "Processing Apps"
                R.string.step_processing_folders -> "Processing Folders"
                R.string.log_processing_app -> "Processing app ${args[0]}..."
                R.string.log_processing_folder -> "Processing folder ${args[0]}..."
                R.string.log_start_migration_mode -> "Starting migration. Mode: ${args[0]}"
                R.string.mode_portable_files -> "Standard Portable Files (.apk, .tar, .json)"
                R.string.mode_encrypted -> "Encrypted (${args[0]})"
                R.string.mode_unencrypted_plaintext -> "Unencrypted (Swift Backup Plaintext)"
                R.string.log_target_account_hash -> "Target Account Hash: ${args[0]}"
                R.string.log_source_directory -> "Source directory: ${args[0]}"
                R.string.log_target_directory -> "Target directory: ${args[0]}"
                R.string.log_migration_finished_detail -> "Migration finished! Successfully processed ${args[0]} apps and ${args[1]} folders."
                R.string.err_source_dir_missing -> "Source directory does not exist or is not a directory: ${args[0]}"
                else -> ""
            }
        ...
        fun log(msg: String) {
            ...
            config.onProgress?.invoke(
                MigrationProgress(
                    currentStep = str(R.string.step_migrating),
                    processedItems = 0,
                    totalItems = 0,
                    currentFileName = "",
                    logMessage = msg
        ...
        if (!config.sourceDir.exists() || !config.sourceDir.isDirectory) {
            val err = str(R.string.err_source_dir_missing, config.sourceDir.absolutePath)
            errors.add(err)
        ...
        log(str(
            R.string.log_start_migration_mode,
            when {
                isPortable -> str(R.string.mode_portable_files)
                isEncrypted -> str(R.string.mode_encrypted, targetUid.orEmpty())
                else -> str(R.string.mode_unencrypted_plaintext)
            }
        ))
        if (!isPortable) log(str(R.string.log_target_account_hash, targetAccountHash))
        log(str(R.string.log_source_directory, config.sourceDir.absolutePath))
        log(str(R.string.log_target_directory, targetBaseDir.absolutePath))
        ...
        for ((pkgName, backupDir) in appBackupDirs) {
            ...
            config.onProgress?.invoke(
                MigrationProgress(
                    currentStep = str(R.string.step_processing_apps),
                    processedItems = completedWork,
                    totalItems = totalWork,
                    currentFileName = "$pkgName ($backupId)",
                    logMessage = str(R.string.log_processing_app, pkgName)
                )
            )
        ...
        for (folderDir in folderBackupDirs) {
            ...
            config.onProgress?.invoke(
                MigrationProgress(
                    currentStep = str(R.string.step_processing_folders),
                    processedItems = completedWork,
                    totalItems = totalWork,
                    currentFileName = folderName,
                    logMessage = str(R.string.log_processing_folder, folderName)
                )
            )
        ...
        log(str(R.string.log_migration_finished_detail, totalApps, totalFolders))
