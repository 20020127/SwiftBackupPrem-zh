...
object FirebaseSyncEngine {
    ...
    fun syncAll(context: Context, prefs: PreferencesManager): SyncResult {
        ...
        if (!prefs.customFirebaseApp || !prefs.syncMetadataToFirebase) {
            return SyncResult(0, 0, 0, listOf(appContext.getString(R.string.sync_disabled)))
        }

        val dbUrl = prefs.firebaseDatabaseUrl.trim()
        if (dbUrl.isBlank()) {
            return SyncResult(0, 0, 0, listOf(appContext.getString(R.string.sync_db_url_empty)))
        }

        val creds = resolveAuthCredentials(context, prefs)
        if (creds == null) {
            return SyncResult(0, 0, 0, listOf(appContext.getString(R.string.sync_auth_unresolved)))
        }
